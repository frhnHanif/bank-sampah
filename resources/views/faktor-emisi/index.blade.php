...
<div class="max-w-6xl mx-auto">
    <div class="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-6"><div><a href="{{ route('kategori.index') }}" class="text-sm text-emerald-700 font-bold"><i class="fa-solid fa-arrow-left mr-1"></i>Jenis Sampah</a><h1 class="text-2xl font-black text-gray-800 mt-2">Master Faktor Emisi</h1><p class="text-sm text-gray-500">Sumber, versi, dan tanggal berlaku disimpan untuk audit. Histori memakai snapshot.</p></div><button onclick="openFactor()" class="bg-emerald-600 text-white rounded-xl px-5 py-3 font-bold"><i class="fa-solid fa-plus mr-2"></i>Tambah Faktor</button></div>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800 mb-5"><i class="fa-solid fa-circle-info mr-2"></i>Jangan memasukkan faktor tanpa sumber yang dapat dipertanggungjawabkan. Faktor hasil migrasi diberi label sumber belum terdokumentasi.</div>
    <div class="space-y-3">
    @forelse($faktor as $item)
        <article class="bg-white border rounded-2xl p-5 flex flex-col lg:flex-row lg:items-center gap-4 {{ !$item->aktif ? 'opacity-60' : '' }}">
            <div class="flex-1"><div class="flex items-center gap-2"><h2 class="font-black text-gray-800">{{ $item->nama_material }}</h2><span class="text-[10px] font-black px-2 py-1 rounded-full {{ $item->aktif?'bg-emerald-100 text-emerald-700':'bg-gray-100 text-gray-600' }}">{{ $item->aktif?'AKTIF':'NONAKTIF' }}</span></div><p class="text-2xl font-black text-emerald-700 mt-2">{{ rtrim(rtrim(number_format($item->faktor_kgco2e_per_kg, 3, ',', '.'), '0'), ',') }} <small class="text-xs">kgCO₂e/kg</small></p><p class="text-xs text-gray-500 mt-2">Sumber: {{ $item->sumber ?: 'Belum dicantumkan' }} | Versi: {{ $item->versi ?: '-' }} | Dipakai {{ $item->kategori_count }} kategori</p></div>
            <div class="flex gap-2"><button onclick='openFactor(@json($item))' class="bg-gray-100 rounded-xl px-4 py-2.5 font-bold text-sm">Edit</button><form action="{{ route('faktor-emisi.toggle',$item) }}" method="POST">@csrf @method('PATCH')<button class="rounded-xl px-4 py-2.5 font-bold text-sm {{ $item->aktif?'bg-red-50 text-red-600':'bg-emerald-50 text-emerald-700' }}">{{ $item->aktif?'Nonaktifkan':'Aktifkan' }}</button></form></div>
        </article>
    @empty <div class="bg-white border rounded-2xl p-10 text-center text-gray-500">Belum ada faktor emisi.</div> @endforelse
    </div>
</div>
