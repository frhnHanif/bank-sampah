            ...
            <div class="lg:col-span-7 space-y-6">
                ...
                <section class="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                    <div class="flex items-center justify-between gap-3 mb-4">
                        <div><h2 class="font-black text-gray-800">Jenis Sampah</h2><p class="text-xs text-gray-500">Harga tidak melekat pada kategori.</p></div>
                        <button type="button" onclick="openQuickCategory()" class="text-sm font-bold text-emerald-700 bg-emerald-50 px-4 py-2.5 rounded-xl hover:bg-emerald-100"><i class="fa-solid fa-plus mr-1"></i> Tambah Jenis Baru</button>
                    </div>
                    <div id="categoryGrid" class="grid grid-cols-2 sm:grid-cols-3 gap-3"></div>
                </section>
            </div>

            <aside class="lg:col-span-5">
                <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden lg:sticky lg:top-28">
                    <div class="p-5 border-b"><h2 class="font-black text-gray-800">Ringkasan Setoran</h2></div>
                    <div id="cartEmpty" class="p-10 text-center text-gray-400"><i class="fa-solid fa-basket-shopping text-3xl mb-3"></i><p>Belum ada item</p></div>
                    <div id="cartList" class="hidden divide-y"></div>
                    <div class="p-5 bg-gray-50 border-t">
                        <div class="flex justify-between items-end mb-4"><span class="text-sm text-gray-500">Total berat terkumpul</span><strong class="text-2xl text-emerald-700"><span id="totalWeight">0,00</span> kg</strong></div>
                        <p class="text-xs text-gray-500 bg-white border rounded-xl p-3 mb-4"><i class="fa-solid fa-clock text-amber-500 mr-1"></i> Status awal: <strong>Menunggu penjualan</strong>. Saldo tidak berubah.</p>
                        <button id="depositSubmit" type="button" onclick="submitDeposit()" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl font-black disabled:opacity-50">Simpan Setoran Pending</button>
                    </div>
                </div>
            </aside>
        </div>
...
let categories = @json($kategori).map(k => ({id:k.id, name:k.nama, classified:!!k.faktor_emisi_id}));
...
function renderCategories() {
    categoryGrid.innerHTML = categories.map(c => `<button type="button" onclick="openWeight(${c.id})" class="text-left border rounded-xl p-4 hover:border-emerald-400 hover:bg-emerald-50"><div class="font-bold text-gray-800">${escapeHtml(c.name)}</div><div class="text-[11px] mt-1 ${c.classified?'text-emerald-600':'text-amber-600'}">${c.classified?'Faktor terklasifikasi':'Belum diklasifikasikan'}</div></button>`).join('');
}
...
function renderCart(){cartEmpty.classList.toggle('hidden',cart.length>0);cartList.classList.toggle('hidden',cart.length===0);cartList.innerHTML=cart.map((i,x)=>`<div class="p-4 flex items-center gap-3"><div class="flex-1"><strong>${escapeHtml(i.nama)}</strong><p class="text-xs text-gray-500">Nilai belum ditentukan</p></div><b>${kg(i.berat)} kg</b><button type="button" onclick="cart.splice(${x},1);renderCart()" class="text-red-400"><i class="fa-solid fa-trash"></i></button></div>`).join('');totalWeight.textContent=kg(cart.reduce((s,i)=>s+i.berat,0));cartData.value=JSON.stringify(cart);}
